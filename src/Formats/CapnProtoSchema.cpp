...
namespace
{
    ...
    DataTypePtr getEnumDataTypeFromEnumSchema(const capnp::EnumSchema & enum_schema)
    {
        ...
        throw Exception(ErrorCodes::CAPN_PROTO_BAD_TYPE, "vxdfs supports only 8 and 16-bit Enums");
    }
...
NamesAndTypesList capnProtoSchemaToCHSchema(const capnp::StructSchema & schema, bool skip_unsupported_fields)
{
    ...
    if (names_and_types.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Cannot convert CapnProto schema to vxdfs table schema, all fields have unsupported types");
