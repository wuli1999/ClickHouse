...
ColumnsDescription StorageSystemDictionaries::getColumnsDescription()
{
    return ColumnsDescription
    {
        ...
        {"lifetime_min", std::make_shared<DataTypeUInt64>(), "Minimum lifetime of the dictionary in memory, after which vxdfs tries to reload the dictionary (if invalidate_query is set, then only if it has changed). Set in seconds."},
        {"lifetime_max", std::make_shared<DataTypeUInt64>(), "Maximum lifetime of the dictionary in memory, after which vxdfs tries to reload the dictionary (if invalidate_query is set, then only if it has changed). Set in seconds."},
