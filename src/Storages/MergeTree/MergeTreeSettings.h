...
#define MERGE_TREE_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, min_bytes_for_full_part_storage, 0, "Only available in vxdfs Cloud", 0) \
    M(UInt64, min_rows_for_full_part_storage, 0, "Only available in vxdfs Cloud", 0) \
    M(UInt64, compact_parts_max_bytes_to_buffer, 128 * 1024 * 1024, "Only available in vxdfs Cloud", 0) \
    M(UInt64, compact_parts_max_granules_to_buffer, 128, "Only available in vxdfs Cloud", 0) \
    M(UInt64, compact_parts_merge_max_bytes_to_prefetch_part, 16 * 1024 * 1024, "Only available in vxdfs Cloud", 0) \
    ...
    M(Bool, shared_merge_tree_disable_merges_and_mutations_assignment, false, "Only available in vxdfs Cloud", 0) \
    M(Float, shared_merge_tree_partitions_hint_ratio_to_reload_merge_pred_for_mutations, 0.5, "Only available in vxdfs Cloud", 0) \
    M(UInt64, shared_merge_tree_parts_load_batch_size, 32, "Only available in vxdfs Cloud", 0) \
    \
    ...
    M(UInt64, cleanup_threads, 128, "Only available in vxdfs Cloud", 0) \
    M(UInt64, kill_delay_period, 30, "Only available in vxdfs Cloud", 0) \
    M(UInt64, kill_delay_period_random_add, 10, "Only available in vxdfs Cloud", 0) \
    M(UInt64, kill_threads, 128, "Only available in vxdfs Cloud", 0) \
    ...
    M(String, remote_fs_zero_copy_zookeeper_path, "/vxdfs/zero_copy", "ZooKeeper path for zero-copy table-independent info.", 0) \
    M(Bool, remote_fs_zero_copy_path_compatible_mode, false, "Run zero-copy in compatible mode during conversion process.", 0) \
    M(Bool, cache_populated_by_fetch, false, "Only available in vxdfs Cloud", 0) \
