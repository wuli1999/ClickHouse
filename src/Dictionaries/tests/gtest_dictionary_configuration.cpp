...
TEST(ConvertDictionaryAST, SimpleDictConfiguration)
{
    ...
    /// source
    EXPECT_EQ(config->getString("dictionary.source.vxdfs.host"), "localhost");
    EXPECT_EQ(config->getInt("dictionary.source.vxdfs.port"), 9000);
    EXPECT_EQ(config->getString("dictionary.source.vxdfs.user"), "default");
    EXPECT_EQ(config->getString("dictionary.source.vxdfs.password"), "");
    EXPECT_EQ(config->getString("dictionary.source.vxdfs.db"), "test");
    EXPECT_EQ(config->getString("dictionary.source.vxdfs.table"), "table_for_dict");
